...
void main() {
    ...
    float pL = texture(prsTex, coords - vec2(delx, 0)).x;
    float pR = texture(prsTex, coords + vec2(delx, 0)).x;
    float pB = texture(prsTex, coords - vec2(0, dely)).x;
    float pT = texture(prsTex, coords + vec2(0, dely)).x;
    
    fragColor = texture(velTex, coords);
    fragColor.xy -= (res.x / res.y) * 0.5 * vec2(pR - pL, pT - pB);
}